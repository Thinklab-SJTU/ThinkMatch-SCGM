...
from utils.evaluation_metric import pck, matching_accuracy, clustering_accuracy
...
def eval_model(model, alphas, dataloader, eval_epoch=None, verbose=False):
    ...
    cluster = torch.zeros(len(classes), device=device)
    ...
    for i, cls in enumerate(classes):
        ...
        acc_match_num_mgm = torch.zeros(1, device=device)
        acc_total_num_mgm = torch.zeros(1, device=device)
        cluster_acc_sum = torch.zeros(1, device=device)
        cluster_total_num = torch.zeros(1, device=device)
        for inputs in dataloader:
            ...
            with torch.set_grad_enabled(False):
                pred = model(data, Ps_gt, Gs_gt, Hs_gt, Gs_ref=Gs_ref, Hs_ref=Hs_ref, KGs=KGs, KHs=KHs, ns=ns_gt, iter_times=2, type=inp_type, pretrain_backbone=False)
            ...
            for s_pred, (gt_idx_src, gt_idx_tgt) in zip(s_pred_list_mgm, indices):
                ...
                gt_perm_mat = torch.bmm(perm_mats[gt_idx_src], perm_mats[gt_idx_tgt].transpose(1, 2))
                _, _acc_match_num, _acc_total_num = matching_accuracy(pred_perm_mat, gt_perm_mat, ns_gt[gt_idx_src])
                acc_match_num_mgm += _acc_match_num
                acc_total_num_mgm += _acc_total_num
            ...
            cluster_acc_sum += clustering_accuracy(pred_cluster, cls_gt_transpose) * batch_num
            cluster_total_num += batch_num
        ...
        accs_mgm[i] = acc_match_num_mgm / acc_total_num_mgm
        cluster[i] = cluster_acc_sum / cluster_total_num
        ...
        if verbose:
            ...
            print('Class {} cluster acc={:.4f}'.format(cls, cluster[i]))
    ...
    for cls, single_acc in zip(classes, accs_mgm):
        print('{} = {:.4f}'.format(cls, single_acc))
    print('average accuracy = {:.4f}'.format(torch.mean(accs_mgm)))

    print('Clustering accuracy')
    for cls, single_acc in zip(classes, cluster):
        print('{} = {:.4f}'.format(cls, single_acc))
    print('average accuracy = {:.4f}'.format(torch.mean(cluster)))
